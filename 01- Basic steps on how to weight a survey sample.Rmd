...
resp.var.num <- which(names(responses) %in% c(resp.id, resp.y, resp.x))

resp.selected.labels <- attributes(responses)$variable.labels[resp.var.num] 

responses <- responses[c(resp.id, resp.x, resp.y)]

attributes(responses)$variable.labels <- resp.selected.labels
