...
Explain penalised logistic regression 

Check how to change intercept for log reg

```{r, echo=TRUE, warning=FALSE, message=FALSE}

response.cvm <- cv.glmnet(x = x.matrix, 
                          y = data.nonresponse.model$response, 
                          family = "binomial",
                          alpha = 0)
...
coef.response.cvm <- coef(response.cvm,s=0, response.cvm$lambda.1se)
...
predicted.glmnet <- predict(response.cvm, 
        x.matrix,
        s=response.cvm$lambda.min,
        type = "response")

predicted.glmnet.auc <- predict(response.cvm, 
        x.matrix,
        s=response.cvm$lambda.min,
        type = "response")
...
data_frame(logistic_regression = predicted.log, 
           cv_glmnet_regression = predicted.glmnet[,1])
...
ci.cvAUC(predicted.glmnet[,1], data.nonresponse.model$response, folds = flds, confidence = 0.95)
