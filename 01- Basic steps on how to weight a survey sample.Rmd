...
After creating the classes, a good practice would be to check for covariate balance. This procedure is explained in pag. 330 of Valliant et al. Units in the same class should have similar values in covariates. At the same time, we would ideally find differencess between classes. In plain words, here we would check if classe are made of homogeneous units and if classes really distinguish different profiles of these. As an example I will do the balance analysis for the ordinal covariates *physa*, *littera* and *vandaa*. For the other covariates, we would need to fit multinomial regressions and interpret a larger number of coefficients.
