...
<<<<<<< HEAD
This is a short introductory guide that shows to weight a survey. Please keep in mind that there are many different ways of weighting a survey. This guide intends to be a practical document and intentionally avoids explaining complex or advanced methods. Instead, I aim at providing 
=======
...
>>>>>>> 33380d27c12184e73f1b468cb1b2a359e6aec1cd
...
This guide follows the structure of the four basic steps in weighting with the addition of two initial sections dealing with data management and presentation of the datasets. Readers can skip the first section and go directly to *Exploring and presenting the dataset* without missing important information to understand the weighting process.   
...
With the ESS UK sampling design we would expect sample probabilities to be related to the type of house and number of people in household where potential respondents live. This is because  
...
data %>%
  ...
  group_by(type) %>%
  summarise(n = n(),
    mean.prob.percentage = mean(prob*100, na.rm = T) %>% round(4),
    median.prob.percentage = median(prob*100, na.rm = T)%>% round(4),
    sd.prob = sd(prob*100, na.rm = T) %>% round(4)) %>%
...
<<<<<<< HEAD
As I mentioned before, base weights should sum up to the entire population from which the sample is drawn (it is not the case for the ESS) and scaled base weights should sum up to all respondents.  
...
Valliant et al (2013) recommend estimating the response propensities and then grouping them in classes. This should avoid extreme weights 
