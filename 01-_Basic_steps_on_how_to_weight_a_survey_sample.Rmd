...
Note that this guide focuses on surveys based on 'probability sample'.
...
1) when all units in the survey frame are approached for the sample or; 2) with certain sampling designs such as 'simple random sampling without replacement' or 'stratified random sampling without replacement' with distribution of sampled units across stratums proportional to the number of units in each stratum. These are usually called 'self-weighted' surveys. 
