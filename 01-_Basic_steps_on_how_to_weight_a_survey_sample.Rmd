...
ess.svydesign <- svydesign(ids = ~ 0, weights = ~final.weight, data = data.calibration)
...
raked.weights <- ess.raked$postStrata[[1]][[1]] %>% attributes() %>% .[["weights"]]

data.calibration$raked.weights <- raked.weights
...
data.calibration %>%
  group_by(gender_age_rec) %>%
  summarise(calibrated.sample.pop = sum(raked.weights)),
age.gender.eurostat)
