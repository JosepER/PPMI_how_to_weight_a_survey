...
c(train.correct.logreg, train.correct.lasso)
...
for(i in 1:length(folds)){
  ...
  success.rate <- c(success.rate, table(temp.predicted, temp.test.data$response) %>% diag() %>% sum()/nrow(temp.test.data) )
  
  
}

  cv.success.log.reg <- success.rate %>% sum()/length(success.rate)

  rm(train.folds, temp.train.data, temp.test.data, temp.log.reg.m, temp.predicted, success.rate)
...
for(i in 1:length(folds)){
  ...
  success.rate <- c(success.rate, table(temp.predicted, temp.test.y) %>% diag() %>% sum()/nrow(temp.test.data) )
  
}

  cv.success.glmnet <- success.rate %>% sum()/length(success.rate)

  rm(t, train.folds, temp.train.data, temp.train.y, temp.test.data,
     tempt.test.y, temp.glmnet.m, temp.predicted, success.rate)  
