...
The total number of weighted units in each of the categories we will use for calibration can be seen in the tables below.
...
Now we import Eurostat data stored in github.

```{r,  echo=TRUE, warning=FALSE, message=FALSE}

age.gender.eurostat <- read_delim("data/Eurostat/Agebygender.csv", delim = ";")

age.gender.eurostat$Value <-  str_replace_all(age.gender.eurostat$Value, pattern = "[.]", "")
  
age.gender.eurostat %>%
  write_csv("data/Eurostat/Agebygender_demo_pjangroup.csv")

age.region <- read_delim("data/Eurostat/Nuts2byage.csv", delim = ";")

age.region %<>% apply(2, function(x){str_replace_all(x, pattern = "[.]", "")})

age.region %<>%
  as_data_frame()

age.region %<>%
  gather(key = years, value = population, -Country)

age.region %>%
  group_by(Country) %>%
  summarise(population_15over = sum(population))

age.gender.eurostat$Value <-  grep(x = age.gender.eurostat$Value, pattern = ".", replacement = "")
...
mygithub.url <- getURL("https://github.com/JosepER/PPMI_how_to_weight_a_survey/blob/master/data/Eurostat/Agebygender.csv")


mygithub.url <- getURL("https://raw.github.com/JosepER/PPMI_how_to_weight_a_survey/blob/master/data/Eurostat/Agebygender.csv")

age.gender.eurostat <- download.file("https://raw.github.com/JosepER/PPMI_how_to_weight_a_survey/blob/master/data/Eurostat/Agebygender.csv","data/Agebygender.csv", method="wininet")


age.gender.eurostat <- download.file("https://github.com/JosepER/PPMI_how_to_weight_a_survey/blob/master/data/Eurostat/Agebygender.csv","data/Agebygender.csv", method="wininet")



load(url(mygithub.url))


eurostat.age.gender <- read_excel(path = paste(mygithub.url, "Age by gender.xlsx", sep = ""),  sheet = "Age_gender")

age.gender.eurostat <-read.csv(text = mygithub.url)

read_delim("https://raw.github.com/JosepER/PPMI_how_to_weight_a_survey/blob/master/data/Eurostat/Agebygender.csv", delim = ";")

read_delim("https://github.com/JosepER/PPMI_how_to_weight_a_survey/blob/master/data/Eurostat/Agebygender.csv", delim = ";")
```


```{r,  echo=TRUE, warning=FALSE, message=FALSE}
svydesign(ids = ~ 0, weights = ~final.weight, data = data)
