...
#### Estimating response propensities using Random Forest.

Last, we are going to try a 'Random Forest' model. This is an algorithm based on the repetition of classification trees. Below you can find the first six predictions using this method.  

```{r,  echo=TRUE, warning=FALSE, message=FALSE}

rf.model <- randomForest(data = data.nonresponse.model %>% select(response, type:vandaa)] x = x.matrix, y = data.nonresponse.model$response )

predicted.rf <- rf.model$predicted

data.nonresponse.model$predicted.rf <- predicted.rf 

predicted.rf %>% head()

```

Comparing the predictions obtained with these three methods (logistic regression, cross-validated lasso and random forest) we observe that they all give relatively similar values. They are all centered around the same mean, which is the proportion of respondents in our subset (except random forest, which has a very small deviation). Lasso regression is the most 'rigid'/'smoother' with a smaller standard deviation in its predictions. Logistic regression is almost always slightly more distant of the mean than the lasso regression. Random forest algorithm seems to fit the sample data more and has the largest standard deviation in the predictions. Here it is important to note that our logistic and lasso regressions included only linear parameters (i.e. no squared coefficients nor interactions). Therefore, there were rather 'rigid' methods. Random forest does allow for interactions between variables. 
...
data.nonresponse.model %<>%
  mutate(predicted.category.log.reg = (predicted.log > 0.5) %>% as.numeric,
         ...
         predicted.category.rf = (predicted.rf > 0.5) %>% as.numeric)
...
train.correct.rf <- table(data.nonresponse.model$response, data.nonresponse.model$predicted.category.rf) %>% diag() %>% sum()/nrow(data.nonresponse.model)

c(train.correct.logreg, train.correct.lasso, train.correct.rf)
