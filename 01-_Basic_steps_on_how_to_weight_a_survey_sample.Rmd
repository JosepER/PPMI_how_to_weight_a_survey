...
Fortunately for us, the probablity of each respondent of being sampled was included in the 7th ESS dataset. In many projects, however, we will have to compute sampling probailities ourselves. A basic but **important test** that should be performed after computing the probabilities is making sure that all probabilities are between 0 and 1.
...
The vast majority of respondents had probability of `r names(table(data$prob))[which.max(table(data$prob))]` or 0.00020306164012234. We see a minority of around 15% of observations with smaller probabilities. 
...
## Step 2: Non-response weights {#nonresponse weights}
